...
<head>
	<script>
        ...
        function fn_saveBoard() {
            ...
            if( $("#div-main-img").children().length === 0 && ( '${PAGE}' === 'news' || '${PAGE}' === 'gallery' )){
            	$.alert("게시글에 이미지를 1개 이상 등록하세요.");
            	return false;
            }
                    ...
                    confirm: {
                        ...
                        , action: function () {
                            ...
                            let data = {
                                ...
                                , EDT_TITLE: $("#title").val()
	                            , EDT_SUB_TITLE : $("#subTitle").val()
                                , PAGE: "${PAGE}"
	                            , MAIN_IMG_PATH : $("#mainImgFilePath").val()
                            };
         ...
         * 이미지 가져오기
         */
        function fn_getCkImg() {
        	const imgReg = /(<img[^>]*src\s*=\s*[\"']?([^>\"']+)[\"']?[^>]*>)/g;
        	
	        $("#div-main-img").empty();
	        
	        let cnt = 0;
        	while(imgReg.test(editor01.getData())){
        		let filePath =  RegExp.$2.trim();
        		let mainIcon = "";
        		cnt++;
                //수정이 아닌 경우 첫번째 이미지만 default 선택
		        if( ( '${boardInfo.EDT_TITLE}' === '' &&  cnt === 1) || $("#mainImgPath").val() === filePath) {
                    mainIcon = '<i class="bi bi-check-circle-fill" style="font-size: 20px;color:#0abf53;"></i>'
                        + '<input type="hidden" id="mainImgFilePath" name="mainImgFilePath" value="' + filePath + '"/>';
                }
		        $("#div-main-img").append('<div class="lb-wrap pointer" onclick="fn_mainImgChk(this);">'
                                            +'<div class="lb-text">'
                                                + mainIcon
                                            +'</div>'
                                            +'<div class="main-img lb-image">'
                                                + '<img class="main-img" src="'+filePath+'"  alt="">'
                                            +'</div>'
                                          +'</div>'
                );
	        }
         ...
         * 대표이미지 지정
         */
        function fn_mainImgChk(element){
	        let imgSrc = $(element).children().next().children().attr("src");

            let mainIcon = '<i class="bi bi-check-circle-fill" style="font-size: 20px;color:#0abf53;"></i>'
                         + '<input type="hidden" id="mainImgFilePath"  name="mainImgFilePath" value="'+imgSrc+'"/>';

        	//대표이미지 아이콘/경로 초기화
        	$(".lb-text").empty();
        	//현재 선택한 이미지에 대표이미지 아이콘 설정
            $(element).children('div:eq(0)').append(mainIcon);
        }
	</script>
</head>
<body>
<!--메인이미지 경로-->
<input type="hidden" id="mainImgPath" value="${boardInfo.MAIN_IMG_PATH}">
		...
		<div class="col-md-9 col-lg-10">
			<form id="frm" method="post" enctype="multipart/form-data">
				...
				<c:if test="${PAGE == 'gallery' || PAGE == 'news'}" >
					<!-- 소제목 -->
					<div class="mb-3">
						<label class="form-label" for="subTitle">소제목</label>
						<input type="text" class="form-control" placeholder="소제목을 입력하세요." id="subTitle" name="subTitle" maxlength="300" value="${boardInfo.EDT_SUB_TITLE}">
					</div>
				</c:if>
